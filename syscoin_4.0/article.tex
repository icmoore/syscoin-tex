...
\section{Block Occupancy Cost (BLOC) Model}

\section{Gas Simulations}
...
\begin{equation}
\begin{split}
\log p(y | x) = \frac{1}{2}(\mathbf{y}-\mathbf{m}_{x})^{T}(\mathbf{K}_{xx} + \mathbf{\Sigma})^{-1}(\textbf{y} - \mathbf{m}_{x}) -\\ \frac{1}{2}|\mathbf{K}_{xx} - \mathbf{\Sigma}| - \frac{n}{2}\log(2\pi)
\end{split}
\end{equation}
